...
#print(exp_param)
  if (mig[1]>0) { ## find the migration event: from mig[1] to mig[2] and get probability of it
    ...
    wait_time = 0
#print(dexp(mig[3],sum(exp_param)))
...
mcmc_plot <- function(data,print_out=0){
  ...
  fitmeanest = loess(data[,30] ~ data[,2])
  fit05est = loess(data[,23] ~ data[,2])
  fit95est = loess(data[,27] ~ data[,2])
  ...
  points(simlam,simlam,type="l",col="grey")
  ...
  fitmeanest = loess(data[,41] ~ data[,3])
  fit05est = loess(data[,34] ~ data[,3])
  fit95est = loess(data[,38] ~ data[,3])
  simlam = seq(0,130,.5)
  preddmean <- predict(fitmeanest, simlam, se=TRUE)
  predd05 <- predict(fit05est, simlam, se=TRUE)
  predd95 <- predict(fit95est, simlam, se=TRUE)
